...
export default function MainCanvas({img, tagline, style}) {
  ...
  useEffect(() => {
    ...
    const pixelSizeAttr = bg.getAttribute("data-pixel-size") ?? "20"
    const inkAttr = bg.getAttribute("data-ink") ?? "#f1a7ce"
...
void main() {
    ...
    /* Animated fbm feed ------------------------------------ */
    vec2 noiseOffset = vec2(-0.15, -0.1); // ajuste comme tu veux
    float feed = fbm2(uv + noiseOffset, uTime * 0.05);
    ...
    // === Animation loop limité à 30 FPS ===
    ...
    // === IntersectionObserver pour pause hors écran ===
  ...
  return (
    <div className="block relative">
      <div className="label flex flex-col justify-center">
        <img className={`aspectm ${style}`} src={img} alt="" />
        <div className="wm">
          <figcaption className="quotes">{tagline}</figcaption>
        </div>
      </div>
      <div
        id="hero_bg"
